...
| EXP-023b positive control on "heading turn or speed change" | the only cached non-STEP prompt is SQUEEZE; turn/speed prompts need the CPU Llama-3-8B encoder (≥ 18 GB RAM), which cannot run beside the current co-tenant job | run SQUEEZE now (protocol drafted; driver built 2026-09-02); add turn/speed phrasings under EXP-027 once the encoder loads | "the same handoff switches [SQUEEZE]" if it does |
